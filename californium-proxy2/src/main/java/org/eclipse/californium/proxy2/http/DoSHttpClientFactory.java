...
public class DoSHttpClientFactory {
   ...
   * @return
   */
  public static CloseableHttpAsyncClient createCustomClient(int numConnections) {
    ...
    Configuration config = getNetworkConfig();
      ...
      .setConnectionManager(createPoolingConnManager(config, numConnections))
      ...
      .setConnectionReuseStrategy(new ConnectionReuseStrategy() {
        ...
        public boolean keepAlive(HttpRequest request, HttpResponse response, HttpContext context) {
          // Force all connections to be kept alive
          return true;
        }
      })
    ...
    long connectTimeoutMillis = config.get(Proxy2Config.HTTP_CONNECT_TIMEOUT, TimeUnit.MILLISECONDS);
		return RequestConfig
      .custom()
      .setConnectionKeepAlive(KEEP_ALIVE)
      .setConnectionRequestTimeout(Timeout.ofMilliseconds(connectTimeoutMillis * 4))
	 ...
	 * @since 3.0 (changed parameter to Configuration)
	 */
	private static PoolingAsyncClientConnectionManager createPoolingConnManager(Configuration config, int numConnections) {
    ...
    return PoolingAsyncClientConnectionManagerBuilder
      .create()
      .setConnectionTimeToLive(KEEP_ALIVE)
      .setConnPoolPolicy(PoolReusePolicy.FIFO)
      // setDnsResolver(DnsResolver dnsResolver)
      .setMaxConnTotal(numConnections)
      .setMaxConnPerRoute(numConnections)
  ...
  private static IOReactorConfig createCustomIOReactorConfig(Configuration config) {
    ...
    Long connectionIdleSecs = config.get(Proxy2Config.HTTP_CONNECTION_IDLE_TIMEOUT, TimeUnit.SECONDS);
